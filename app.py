    ...
    with col2:
        ...
        if not pg_file:
            st.button("📁 Descargar carpetas (ZIP)", disabled=True, use_container_width=True,
                       help="Sube pg.txt para habilitar esta descarga")
        elif not zip_valido:
            st.button("📁 Descargar carpetas (ZIP)", disabled=True, use_container_width=True,
                       help="El ZIP salió corrupto, vuelve a procesar")
        else:
            b64_zip = base64.b64encode(zip_bytes).decode()
            href = (
                f'<a download="{nombre_zip}" href="data:application/zip;base64,{b64_zip}" '
                f'style="display:inline-block;width:100%;text-align:center;padding:0.5em 0;'
                f'background-color:#1e293b;color:white;font-weight:bold;border-radius:8px;'
                f'text-decoration:none;">📁 Descargar carpetas (ZIP)</a>'
            )
