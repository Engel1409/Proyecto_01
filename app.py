...
with col_up1:
    uploaded_files = st.file_uploader("Sube PDF", type="pdf", accept_multiple_files=True, key=f"p_{st.session_state.reset_id}")
with col_up2:
    pg_file = st.file_uploader("Sube pg.txt (opcional)", type="txt", key=f"g_{st.session_state.reset_id}")
with col_up3:
...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        # ... (Lógica de extracción de items igual que tenías) ...
    ...
    st.divider()
    st.markdown(f'<a href="data:application/zip;base64,{b64_zip}" download="POLIDATA_{datetime.now().strftime("%Y%m%d")}.zip"><button style="width:100%; background:#1e293b; color:white; border:none; padding:10px; border-radius:8px; font-weight:bold;">📁 Descargar ZIP</button></a>', unsafe_allow_html=True)
